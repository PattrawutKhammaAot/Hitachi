...
class _PMdailyHold_ScreenState extends State<PMdailyHold_Screen> {
                              ...
                              rows: [
                                ...
                                DataRow(cells: [
                                  DataCell(
                                      Center(child: Label("Operator Name."))),
                                  DataCell(Label(
                                      "${PMList.where((element) => element.ID == _index.first).first.OPERATOR_NAME}"))
                                ]),
                                DataRow(cells: [
                                  DataCell(Center(child: Label("Checkpoint"))),
                                  DataCell(Label(
                                      "${PMList.where((element) => element.ID == _index.first).first.CHECKPOINT}"))
                                ]),
                                DataRow(cells: [
                                  DataCell(Center(child: Label("Status"))),
                                  DataCell(Label(
                                      "${PMList.where((element) => element.ID == _index.first).first.STATUS}"))
                                ]),
                                DataRow(cells: [
                                  DataCell(Center(child: Label("Start Date "))),
                                  DataCell(Label(
                                      "${PMList.where((element) => element.ID == _index.first).first.DATEPM}"))
                                ]),
                              ])
                        ],
                      )),
  ...
  void _sendDataServer() async {
    _index.forEach((element) async {
      var row = PMList.where((value) => value.ID == element).first;
